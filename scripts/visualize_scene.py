...
def draw_frame(ax, x_center=-500, y_center=650, h=2100):
    """Draw blue palletizing frame - left/back of robot"""
    w, d = 950, 750
    
    # Vertical posts
    for dx in [-w/2 + 35, w/2 - 35]:
        for dy in [y_center - d/2 + 35, y_center + d/2 - 35]:
            draw_cylinder(ax, (x_center + dx, dy, h/2), [0,0,1], 38, h, FRAME_BLUE)
    
    # Horizontal beams
    draw_box(ax, (x_center, y_center - d/2 + 35, h - 45), (w, 70, 90), FRAME_BLUE, 0.88)
    draw_box(ax, (x_center, y_center + d/2 - 35, h - 45), (w, 70, 90), FRAME_BLUE, 0.88)
    draw_box(ax, (x_center - w/2 + 35, y_center, h - 45), (70, d, 90), FRAME_BLUE, 0.88)
    draw_box(ax, (x_center + w/2 - 35, y_center, h - 45), (70, d, 90), FRAME_BLUE, 0.88)
    
    # Base platform
    draw_box(ax, (x_center, y_center, 50), (w - 80, d - 80, 100), FRAME_BLUE, 0.80)
    
    # Mesh panels (semi-transparent)
    mesh_color = (0.35, 0.65, 0.95, 0.18)
    draw_box(ax, (x_center, y_center + d/2 - 18, h/2), (w - 100, 36, h - 220), mesh_color[:3], mesh_color[3])
    draw_box(ax, (x_center - w/2 + 18, y_center, h/2), (36, d - 100, h - 220), mesh_color[:3], mesh_color[3])
    draw_box(ax, (x_center + w/2 - 18, y_center, h/2), (36, d - 100, h - 220), mesh_color[:3], mesh_color[3])
