...
<p align="center">
  <img src="docs/images/S50_palletizing_v7.gif" alt="HR_S50-2000 码垛工作站 3箱连续码垛演示" width="700">
  <br>
  <em>HR_S50-2000 码垛工作站 v7.0 — 3箱连续码垛演示 (20帧)</em>
</p>

```matlab
% 运行码垛工作站仿真 (3箱连续码垛)
run('testS50_Palletizing.m')
```

**testS50_Palletizing.m** 实现了一个完整的 HR_S50-2000 码垛工作站仿真，包含传送带取箱、笼式框架内码垛的全流程演示。
...
| 参数 | 值 | 说明 |
|------|------|------|
| 电箱 (cab) | 0.55×0.65×0.80m | 机器人基座平台，位于原点 |
| 蓝色笼式框架 | 1.20×1.00×2.00m | 中心 (0.00, 1.23m)，-Y面开口朝向机器人 |
| 蓝色托盘 | 1.00×0.80×0.55m | 框内地面，颜色 `[0.20, 0.45, 0.80]` |
| 传送带 | 0.55×2.00m (H=0.75m) | 中心 (0.95, -0.30m)，皮带面 z=0.815m |
| 箱子 | 0.40×0.30×0.25m | 棕色纸箱，箱顶 z≈1.065m |
| 安全间距 | ≥ 0.40m | 电箱↔框架/传送带均保持 0.40m |

#### 传送带箱子位置 (全部在 -Y 半轴)

| 箱号 | 世界坐标 (x, y, z) | 说明 |
|------|------|------|
| #1 | (0.950, **-0.450**, 1.065) | 最先被取，靠近机器人 |
| #2 | (0.950, **-0.900**, 1.065) | 第二个被取 |
| #3 | (0.950, **-1.350**, 1.065) | 最远，最后被取 |

传送方向: **-Y → +Y** (从远端向机器人方向传送)

#### 蓝框内码垛布局 (紧密排列)

| 箱号 | 放置位置 (x, y, z) | 层号 | 说明 |
|------|------|------|------|
| #1 | (-0.200, 1.225, 0.800) | 第1层 | 左侧 |
| #2 | (+0.200, 1.225, 0.800) | 第1层 | 右侧 (与 #1 紧密排列) |
| #3 | (0.000, 1.225, 1.050) | 第2层 | 居中叠放 |

第1层: 两个箱子沿 X 方向紧密排列 (总宽 0.80m < 托盘 1.00m)，第2层居中叠放以提高容积率。
...
1. **悬停取箱位** — 移动到传送带箱子上方
2. **下降取箱** — TCP 对准箱顶，IK 求解到精确位置
3. **提升** — 携带箱子抬升，传送带上箱子减少
4. **转向放箱** — 旋转 J1 朝向蓝色框架
5. **下降放置** — TCP 到达码垛目标位置
6. **释放后退** — 箱子放入框内，框内箱子增加

3 箱 × 6 步 + 初始待机 + 任务完成 = **20 帧**

#### 仿真结果

| 指标 | 结果 | 说明 |
|------|------|------|
| 碰撞检测 | **0/20 ✅** | 全部 20 帧无自碰撞和环境碰撞 |
| 取箱 Z 精度 | 1~3 mm | 箱顶目标 z=1.065m |
| 放箱 Z 精度 | 0~4 mm | L1: z=0.800m, L2: z=1.050m |
| TCP 朝下偏差 | 0°~4° | 末端执行器保持竖直向下 |
| IK 求解 | 6/6 成功 | 3个取箱 + 3个放箱位置全部可达 |
| 关键腕部构型 | q4 ≈ -135°~-152° | 低 z 目标所需的大腕角 |

#### IK 求解详情

| 动作 | J1 | 距离 r | z_base | 误差 |
|------|------|------|------|------|
| 取箱1 | +154.7° | 1.051m | +0.265m | 1.4mm |
| 取箱2 | +136.5° | 1.309m | +0.265m | 2.3mm |
| 取箱3 | +125.1° | 1.651m | +0.265m | 2.8mm |
| 放箱1 | -80.7° | 1.241m | +0.000m | 2.3mm |
| 放箱2 | -99.3° | 1.241m | +0.000m | 2.3mm |
| 放箱3 | -90.0° | 1.225m | +0.250m | 4.0mm |

> **注**: XY 平面偏差约 257mm 是 DH 参数 d2=336.2mm 导致的固有侧向偏移，不影响实际夹取操作（工业吸盘/夹爪覆盖范围足够）。
...
│   ├── trajectory_2/
│   └── trajectory_3/
├── S50_palletizing/             # 码垛工作站仿真 (v7.0)
│   ├── pose_01~20.png          # 20帧码垛过程图片
│   └── palletizing_anim.gif    # 码垛动画 (3箱连续)
├── AllModels_sim/              # 多模型对比
