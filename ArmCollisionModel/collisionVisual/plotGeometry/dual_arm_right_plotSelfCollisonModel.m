function outputStruct = dual_arm_right_plotSelfCollisonModel(Tf_tree, params, toolparams)
    % 定义输出结构体
    outputStruct = struct();
    view(3)
    % 获取输入的变换矩阵
    ...
    % 绘制低臂部分
    [la1, la2] = plotCapsule(T03, params.r.lowerArm.start(1), params.r.lowerArm.start(2), params.r.lowerArm.start(3), ...
        params.r.lowerArm.end(1), params.r.lowerArm.end(2), params.r.lowerArm.end(3), params.r.lowerArm.radius);
    outputStruct.lowerArm_la1 = la1;
    outputStruct.lowerArm_la2 = la2;
    hold on;
    % 绘制肘部分
    [e1, e2] = plotCapsule(T04, params.r.elbow.start(1), params.r.elbow.start(2), params.r.elbow.start(3), ...
        params.r.elbow.end(1), params.r.elbow.end(2), params.r.elbow.end(3), params.r.elbow.radius);
    outputStruct.elbow_e1 = e1;
    outputStruct.elbow_e2 = e2;
    hold on;
    % 绘制上臂部分
    [ua1, ua2] = plotCapsule(T05, params.r.upperArm.start(1), params.r.upperArm.start(2), params.r.upperArm.start(3), ...
        params.r.upperArm.end(1), params.r.upperArm.end(2), params.r.upperArm.end(3), params.r.upperArm.radius);
    outputStruct.upperArm_ua1 = ua1;
    outputStruct.upperArm_ua2 = ua2;
    hold on;
    % 绘制腕部分
    wc = plotBall(T06, params.r.wrist.offset(1), params.r.wrist.offset(2), params.r.wrist.offset(3), params.r.wrist.radius);
