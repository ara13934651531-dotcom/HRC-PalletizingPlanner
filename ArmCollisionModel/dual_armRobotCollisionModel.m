classdef dual_armRobotCollisionModel < handle
    properties
        robot;  % 机器人对象
        stlPath;  % 模型路径
        urdfPath;
    ...
    methods
        % 构造函数
        function obj = dual_armRobotCollisionModel(robotType)
            % 根据机器人类型设置模型路径和导入机器人
...
%            "model\meshes\tool\Paint_gun4.stl";
             obj.robot.Bodies{12}.addVisual("Mesh",toolPath);
         end


        
        % 显示机器人模型
        function showRobot(obj, robName)
